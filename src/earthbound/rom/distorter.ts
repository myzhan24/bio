import { HORIZONTAL, HORIZONTAL_INTERLACED, SNES_HEIGHT, SNES_WIDTH, VERTICAL } from '../constants';
...
export class Distorter {
  ...
  constructor(bitmap) {
    ...
    this.bitmap = bitmap;
    /* NOTE: Another discrepancy from Java: These values should be "short" and must have a specific precision.
    This seems to affect backgrounds with distortEffect === HORIZONTAL */
    this.C1 = 1 / 512;
  ...
  getAppliedOffset(y, distortionEffect) {
    ...
    switch (distortionEffect) {
      ...
      case VERTICAL:
        /* Compute L */
        return this.mod(floor(s + y * this.compression), 256);
    }
  }
    ...
    for (y = 0; y < SNES_HEIGHT; ++y) {
      ...
      for (x = 0; x < SNES_WIDTH; ++x) {
        ...
        if (distortionEffect === HORIZONTAL || distortionEffect === HORIZONTAL_INTERLACED) {
          dx = this.mod(x + offset, SNES_WIDTH);
        }
